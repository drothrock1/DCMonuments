...
- (void)viewDidLoad
{
	...
	[scrollView setContentSize:CGSizeMake(320, 3650)];
...
- (void) prevController {
	PageElevenViewController *pageElevenVC = [[PageElevenViewController alloc] initWithNibName: @"PageElevenViewController" bundle:nil];
	...
	pageElevenVC.hidesBottomBarWhenPushed = YES;
	[navigationStack addObject:pageElevenVC];
	
	[pageElevenVC release];
...
-(void)updateViewForPlayerInfo {
	...
	//NSLog(@"duration text in updateviewforplayerinfo method: %i",self.duration.text);
	self.progressBar.maximumValue = player.duration;
	//NSLog(@"duration progressbar maximumvalue updateviewforplayerinfo method: %i",self.progressBar.maximumValue);
}
